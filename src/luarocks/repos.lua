...
local function backup_existing(target)
...
function repos.deploy_files(name, version, wrap_bin_scripts, deps_mode)
   ...
   local function install_binary(source, target)
      if wrap_bin_scripts and fs.is_lua(source) then
         backup_existing(target .. (cfg.wrapper_suffix or ""))
   ...
   if rock_manifest.bin then
      ...
      repos.recurse_rock_manifest_entry(rock_manifest.bin, function(file_path)
         ...
         if mode == "nv" and cur_name then
            local cur_nvers_bin, cur_vers_bin = get_deploy_paths(cur_name, cur_version, "bin", file_path, repo)
            table.insert(renames, { src = cur_nvers_bin, dst = cur_vers_bin, suffix = cfg.wrapper_suffix })
         end
         table.insert(installs, { fn = install_binary, src = source, dst = mode == "nv" and paths.nv or paths.v })
      end)
   end
   ...
   if rock_manifest.lua then
      ...
      repos.recurse_rock_manifest_entry(rock_manifest.lua, function(file_path)
         ...
         if mode == "nv" and cur_name then
            local cur_paths = get_deploy_paths(cur_name, cur_version, "lua", file_path:gsub("%.lua$", "." .. cfg.lib_extension), repo)
            table.insert(renames, { src = cur_paths.nv, dst = cur_paths.v })
            cur_paths = get_deploy_paths(cur_name, cur_version, "lib", file_path, repo)
            table.insert(renames, { src = cur_paths.nv, dst = cur_paths.v })
         end
         table.insert(installs, { fn = move_lua, src = source, dst = mode == "nv" and paths.nv or paths.v })
      end)
   end
   ...
   if rock_manifest.lib then
      ...
      repos.recurse_rock_manifest_entry(rock_manifest.lib, function(file_path)
         ...
         table.insert(installs, { fn = move_lib, src = source, dst = mode == "nv" and paths.nv or paths.v })
      end)
   end
